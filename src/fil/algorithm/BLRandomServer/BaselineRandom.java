...
public class BaselineRandom  {
	...
	public BaselineRandom() {;
		//numSFCTotal = 0;
	}
	
	public static void baselineMapping(int numRequest, int i, int numChain, Topology topo, LinkedList<Rpi> listRpi, 
...
//						set position for all chains of a pi
					...
					for(int numSFC = 0; numSFC < numChain; numSFC++ ) { // initialize SFC list
						...
						sfc.setEndTime(StdRandom.exp(0.08)); // leave after 2 hours
					...
					if(totalBandwidth > listRpi.get(i).getRemainBandwidth()) {
						System.out.println("bw is not enough");
						 ...
						 // reduce number of chain because bandwidth is not enough
...
//				count_map++;
				...
				if(listSFCLeave.size() != 0) {
					...
					while(iter.hasNext()) {
						...
						if(sfc.getEndTime() <= numRequest) {
							if(flagLeave == 0) { // if there exists a chain runs out of time
								mappingServer.getLinkMapping().reverseLinkMapping(topo, listSFCLeave);
								mappingServer.getServiceMapping().reverseServer(listSFCLeave); // reset at server
					...
					if(flagLeave != 0) {
						...
						baselineMapping(numRequest, i, numChainRemap, topo, listRpi, listRpiSFC, mappingServer, listRpiSFCLeave, networkRouting);
				...
				///JOIN --PROCESS ////////////////////////////////////////////////////////////////////////
				baselineMapping(numRequest, i, numChain, topo, listRpi, listRpiSFC, mappingServer, listSFCFinal, networkRouting);
				///////////////////////////////////////////////////////////////////////////////////
...
//					run mapping server for final listSFC
	...
	//				System.out.println("sumLoadPi ..." +sumLoadPi+" finalCpuPi " + finalCPUPi + "\n");
					totalChainSystem_temp += finalNumChain; // num of accepted request all over the systen
										//totalChainLeave.add(chainLeave);
...
//		}/////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		try {
			...
			write_double("./PlotBaselineRand/linklistCoreAggBLRD.txt",listCoreAgg);
			write_double("./PlotBaselineRand/linklistAggEdgeBLRD.txt",listAggEdge);
			write_double("./PlotBaselineRand/linklinkPhyEdgeBLRD.txt",linkPhyEdge);
			write_double("./PlotBaselineRand/linkBandwidthBLRD.txt",linkBandwidth);
			write_double("./PlotBaselineRand/serverUtilizationBLRD.txt",serverUtilization);
			write_integer("./PlotBaselineRand/totalChainLeaveBLRD.txt",totalChainLeave);
			write_integer("./PlotBaselineRand/listServerUsedBLRD.txt",listServerUsed);
			write_integer("./PlotBaselineRand/requestRandomBLRD.txt",requestRandomReceive);
			write_integer("./PlotBaselineRand/totalDecOffloadBLRD.txt",totalDecOffload);
			write_integer("./PlotBaselineRand/totalDenOffloadBLRD.txt",totalDenOffload);
			write_double("./PlotBaselineRand/totalPowerSystemBLRD.txt",totalPowerSystem);
			write_double("./PlotBaselineRand/totalEdgePowerSystemBLRD.txt", totalEdgePowerSystem);
			write_double("./PlotBaselineRand/totalLoadEdgeBLRD.txt",totalLoadEdge);
			write_double("./PlotBaselineRand/totalBwEdgeBLRD.txt",totalBwEdge);
			write_double("./PlotBaselineRand/totalChainAcceptanceBLRD.txt",totalChainAcceptance);
			write_double("./PlotBaselineRand/sumLoadNumPiBLRD.txt", sumLoadNumPi);
			write_double("./PlotBaselineRand/sumBwNumPiBLRD.txt", sumBwNumPi);
			write_integer("./PlotBaselineRand/totalChainSystemBLRD.txt",totalChainSystem);
			write_integer("./PlotBaselineRand/totalChainActiveBLRD.txt",totalChainActive);
			write_integer("./PlotBaselineRand/totalChainRejectBLRD.txt",totalChainReject);
			write_integer("./PlotBaselineRand/numChainRequestBLRD.txt",numChainRequest);
			write_integer("./PlotBaselineRand/numChainAcceptBLRD.txt",numChainAccept);
		} catch (IOException e) {
