...
public class ResourceMappingReserve  {
	final static int NUM_PI = 200;
	final static int REQUEST = 30;
	final static int K_PORT_SWITCH = 10; // 5 server/edge switch
...
//		
		...
		ArrayList<Integer> numServiceMigrate = new ArrayList<Integer>();
...
//				count_map++;
				...
				//mappingServer.getServiceMapping().resetRpiSFC(listPossibleLeave, listLinkBandwidth);
				//lay so service cloud luc truoc
				int numCloudService = 0;
				
				for(SFC sfc : listRpiSFC.get(listRpi.get(i))) {
					for(int index = 4; index >= 2; index --) {
						if(!sfc.getService(index).isBelongToEdge()) {
							numCloudService++;
						}
					}
				}
				...
				//lay so service ow cloud luc sau
				for(SFC sfc : listSFCFinal) {
					for(int index = 4; index >= 2; index --) {
						if(!sfc.getService(index).isBelongToEdge()) {
							numCloudService--;
						}
				...
				System.out.println("Number of service migrate to edge " + numCloudService);
				numServiceMigrate.add(numCloudService);
				...
				if(totalChainSystem_temp <= 300) {
					...
					int numService = 0;
					for(int index = 0; index < numServiceMigrate.size(); index++) {
						numService += numServiceMigrate.get(index);
					}
					System.out.println("Total number of service migrate to edge " + numService);
					break LEAVE_LOOP;
				}
			}
		...
		try {
			write_double("./PlotReserve/serverUtilizationRS.txt",serverUtilization);
			write_integer("./PlotReserve/listServerUsedRS.txt",listServerUsed);
			write_integer("./PlotReserve/requestRandomRS.txt",requestRandomReceive);
			write_integer("./PlotReserve/totalDecOffloadRS.txt",totalDecOffload);
			write_integer("./PlotReserve/totalDenOffloadRS.txt",totalDenOffload);
			write_double("./PlotReserve/totalPowerSystemRS.txt",totalPowerSystem);
			write_double("./PlotReserve/totalEdgePowerSystemRS.txt", totalEdgePowerSystem);
			write_double("./PlotReserve/totalLoadEdgeRS.txt",totalLoadEdge);
			write_double("./PlotReserve/totalBwEdgeRS.txt",totalBwEdge);
			write_double("./Graph/DropSFC/totalChainAcceptanceRS.txt",totalChainAcceptance);
			write_double("./PlotReserve/sumLoadNumPiRS.txt", sumLoadNumPi);
			write_double("./PlotReserve/sumBwNumPiRS.txt", sumBwNumPi);
			write_integer("./PlotReserve/totalChainSystemRS.txt",totalChainSystem);
			write_integer("./PlotReserve/totalChainRejectRS.txt",totalChainReject);
		} catch (IOException e) {
